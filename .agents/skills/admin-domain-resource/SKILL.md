...
- All visible copy uses `t()`; new keys go into both `zh` and `en` maps in `frontend/src/lib/i18n.tsx`.
...
7. Run focused tests, then `cd frontend && npm run lint`, `npm run build`, and `npm test -- --run`.
...
- [ ] `frontend/src/lib/i18n.tsx` `zh` / `en` maps are symmetric for any new user-visible keys.
