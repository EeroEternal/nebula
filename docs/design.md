...
| **Vocabulary** | [`design/tokens.md`](design/tokens.md) + `frontend/src/index.css` + `frontend/src/components/ui/*` | Use named tokens and shared components. **Do not invent** colors, radii, or a parallel class set |
...
Applies to `frontend/src/components/views/*`, `frontend/src/components/*`, and `frontend/src/components/ui/*`.
Third-party internals and code-highlight themes are out of scope for wrapper-controlled styling only.
Existing pages may drift; **new UI work follows this spec**. Missing primitives are copied from console-kit into `frontend/src/components/ui`, not invented.

## Hard rules

0. **Stack lock.** Product UI is `frontend/` (React + `frontend/src/components/ui`, tokens in `frontend/src/index.css`). **Never** add a parallel page as raw `index.html` + inline CSS/JS, a second Vite/Next app, or CDN Bootstrap/Tailwind Play. This kit **ships those components**. Extend it; do not replace it. Vercel-style standalone HTML is for *out-of-repo* artifacts with a public stylesheet — not for this codebase. Mechanical gate: `scripts/check_ui_stack.sh`.
