...
| 文档 | 用途 |
|------|------|
| [ha_roadmap.md](./ha_roadmap.md) | 执行清单与 DoD（主体已完成） |
...
| [optimization.md](../../arch/optimization.md) | 全项目排期真源 |
| [architecture.md](../../arch/architecture.md) | 架构原则 |

约定：`report-*.md` 放演练报告；`runbook-*.md` 放应急手册。  
仓库：`docker-compose.ha.yml`、`deploy/ha/`、`scripts/phase_d_ha_drill.sh`。

**状态：** 接入/调度 HA 真机 PASS；生产 etcd 三节点**暂缓**。
