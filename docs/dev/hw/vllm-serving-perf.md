...
### 12.3 逐请求对齐（request_id）

客户端带 `x-nebula-request-id`，gateway 分段日志带同一 id；C=32/N=256，256 条全部 join 成功（µs）：

| 指标 | p50 | p90 | **p99** | max |
|------|-----|-----|---------|-----|
| A 直连 client TTFT | 171920 | 239270 | 261650 | 271820 |
| C 经 gateway client TTFT | 169440 | 198660 | 209260 | 209540 |
| **C gateway 内部** `recv→1st chunk` | 162851 | 186806 | 201708 | 203517 |
| **C 网络段** = clientTTFT − gateway内部 | 6340 | 11145 | 39218 | 44330 |

**最终判定**：

1. **C 的 client TTFT ≈ gateway 内部 + 网络段**，**逐请求成立**。
2. **gateway 内部完全由引擎主导**（§12.2：gateway 自身处理 ~0.1ms）。
3. 网络段 p50 6ms / **p99 39ms** → 唯一增加的是**客户端 ↔ 网关的网络段**（本测试为 pod→host）。
4. 本轮 **A 直连 p99（262ms）反而高于 C（209ms）** → 跨轮 A/C 总时长被引擎波动主导，**不可靠**；可靠的是逐请求分解。

→ **Nebula gateway 的处理开销 ≈ 0**；早期「+18% P99」是**网络路径 + 引擎跨轮波动**的组合，不是 Nebula 处理。
