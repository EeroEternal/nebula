...
| Engine Adapter | 部分实现 | 支持启停、健康、指标；缺能力发现和严格配置校验 |
| Serving Cell | 未实现 | 尚无 `ServingCell`、`ServingTopology` 或原生 Ingress 注册闭环 |
| 引擎可观测 | 部分实现 | 已有少量 pending / KV / cache 与 Gateway / Router 请求指标 |
| 镜像管理 | 部分实现 | 已有注册、预拉取和 GC；尚未形成硬件兼容矩阵和自动选择 |
| 加速器台账 | 部分实现 | 主要是 NVIDIA 瞬时状态；缺型号、驱动、平台和历史画像 |
| SLO / 成本治理 | 未形成闭环 | 有指标和阈值草案，无服务目标对象、评估器和成本模型 |
| Benchmark / 推荐 | 未实现 | 无标准任务、画像库和推荐证据链 |
| 多租户 | 部分实现 | 有鉴权、角色、限流和审计；缺租户配额、优先级和成本归因 |
...
- 定义 `EngineCapability`、`ServingTopology`、`CellIngress` 的 JSON / Rust 契约草案。
...
- 为 engine scrape 增加成功、失败、超时、解析失败和 stale 指标。
- 修正 `kv_cache_used_bytes` / `kv_cache_free_bytes` 当前实际为比例刻度而非真实字节的命名或展示语义。
- 校验 Gateway → Router → Engine 的 W3C TraceContext 和 OTLP endpoint。
- 审计 BFF、Gateway、Router、Scheduler 的指标名称、label 和实际数据源。
- 建立“支持 / 不支持 / 未知”三态，禁止用 `0` 表示上游不支持。
...
- `EngineCapability` 描述引擎版本、协议、模型类型、TP / DP、LoRA、结构化输出、KV Connector 和观测能力。
- Engine Adapter 增加能力发现、配置校验、健康语义、指标映射和版本兼容接口。
- 未知 `engine_type` 必须显式报错，不再静默回退 vLLM。
- 配置在创建 Deployment 时校验，避免等到 Node 启动阶段才失败。
- 能力声明区分“静态兼容表”“运行时发现”和“人工覆盖”，并记录来源。
...
详细指标实施遵循 [`engine_observability_plan.md`](engine_observability_plan.md)。
...
- 完成 SLO / 告警 runbook，明确 abort / 主动 Drain 不计入 5xx 错误预算。
...
- 在 `scripts/` 下建立标准 benchmark workload、执行器和结果 schema。
- 建立 `模型 × 引擎 × 版本 × 硬件 × 参数` 性能画像。
- 结果至少包含 TTFT、TPOT、吞吐、错误率、显存和单位有效 token 成本。
- 推荐 API 输入模型、SLO、预算和可用硬件，输出候选方案、证据和置信度。
- 无足够画像时返回“数据不足”，不静默选择默认引擎。
- 支持候选方案 canary、流量权重、对照评估和一键回滚。
- 线上 SLI 只校正同类负载画像，避免将不同 prompt / output 分布直接比较。

### 验收门槛

- 至少两种模型、vLLM / SGLang 和一个硬件档位的 benchmark 可重复执行。
- 推荐结果可追溯到 benchmark run、软件版本和线上观测窗口。
- canary 未达 SLO 时停止放量并恢复上一稳定版本。
- 回滚后 Deployment、Placement、流量和观测事件保持一致。
- 推荐服务故障不影响手工声明式部署。
...
- 建立租户实体及 token / API key 映射。
- `ExecutionContext` 全链路携带 `tenant_id`、priority、deadline 和预算信息。
- 支持租户级 RPS、并发、token 和模型访问配额。
- 准入控制在 Gateway / Router 边界执行，不把租户策略推入原生引擎。
- 建立租户、模型、引擎版本和 Cell 维度的用量与成本归因。
- 所有配额、策略、发布和治理动作进入审计日志。
- 控制台提供租户 SLO、用量、成本和拒绝原因视图。

### 验收门槛

- 一个租户耗尽配额不会阻塞其他租户。
- 限流和拒绝具有稳定错误码、审计记录和指标。
- 高基数租户信息不进入 Prometheus label；明细通过 trace / 审计查询。
- 成本数据能追溯 token、引擎 / 硬件价格配置和统计窗口。
- 关闭多租户功能时保持现有单 token 模式兼容。
...
- [`engine_observability_plan.md`](engine_observability_plan.md) 继续负责 P0 / P4 中的引擎可观测细节。
