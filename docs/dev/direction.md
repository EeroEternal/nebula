...
| **P0** | **引擎级有界公平队列**（[`queue-admission.md`](queue-admission.md)） | 实测 C=256 时 P99 2.5s（§1） | 新增能力 |
...
| 优化 | 依据 | 预期 |
|------|------|------|
| **引擎级有界公平队列**（不做 AIMD，见 [`queue-admission.md`](queue-admission.md)） | 实测 C=8→256，P99 TTFT 81ms→**2464ms**，甜点 C≈8–16（§1） | P99 从秒级回落 |
...
| **P0** | 引擎级有界公平队列准入（[`queue-admission.md`](queue-admission.md)） | 性能+产品 | 实测 C=256 P99 2.5s |
