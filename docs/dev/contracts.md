...
| C5 | Tooling | 请求含 tools 时：支持则经 **UniGateway** 转换透传；不支持则稳定 `unsupported`，不静默丢字段 | UG `tools`/`tool_choice` 转换 + `ToolCallingCapabilities`；Nebula 映射引擎 Capability 后拒答 |
| C6 | KV/指标单位 | 对外统一字段；引擎差在 scrape | `EndpointStats.kv_cache_usage` |

## 协议适配归属

OpenAI chat / Responses / Anthropic Messages / tooling 的解析与互转 → **[UniGateway](https://github.com/EeroEternal/unigateway)**（Gateway 依赖 `unigateway-sdk` 2.7 `conversion`；同线 protocol/core）。Nebula Gateway 只做鉴权、租户、注入 model、调 Router；不自建第二套方言转换。
