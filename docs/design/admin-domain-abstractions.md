...
- `frontend/src/lib/api-types/common.ts`：共享 API 响应和分页类型。
- `frontend/src/lib/api-types/tenancy.ts`：Organization、ProjectSummary、ProjectOption。
- `frontend/src/lib/api-types/api-keys.ts`：ApiKeyListItem、ApiKeyDetail、ApiKeySecretResult。
- `frontend/src/lib/api-types/billing.ts`：BillingQuery、BillingOverview、BillingTrendPoint。
...
`frontend/src/lib/navigation.ts` 只负责生成高频、可复用的目标 URL；页面仍负责读取参数、校验资源是否存在以及在消费后使用 `replace` 清理一次性参数。
...
1. 新建 `frontend/src/lib/api-types/`。
...
- 新增 `frontend/src/lib/api-types/`，集中 common、tenancy、API Key、User 和 Billing 类型。
...
- 新增 `frontend/src/lib/navigation.ts`，统一日志 API Key、Provider/Model/Chat、Route、API Key 和 Organization 的高频跳转 URL 及 query 编码；页面仍负责消费和清理一次性参数。
- 明确区分 `routeId`（API Key 创建时预选路由）与 `route_id`（日志/资源过滤参数），并在 Route 创建成功流程提供带路由和项目上下文的 API Key 创建入口。
- 未改变后端 endpoint、权限规则或统计口径。
- 已通过 `cd frontend && npm run lint`、`cd frontend && npm run build` 和 `cd frontend && npm test -- --run`；新增 Billing overview/trend、API Key quota/model-limit、Provider Type/Model Discovery、Route、Conversation、Logs、Queue、Analytics、Mock Services、Filter、Quota、System、Provider Insights 与 User resource 查询契约测试。
...
- `frontend/src/components/services/types.ts`
- `frontend/src/hooks/useAdminCatalog.ts`
- `frontend/src/lib/billing-filters.ts`
