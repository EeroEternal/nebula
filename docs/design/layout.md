...
Sidebar entries: `frontend/src/lib/nav.ts` (or product equivalent). Do not invent a second menu.

### User menu (top-right, after login)

Canonical: `frontend/src/components/layout/user-menu.tsx`. Trigger is a 32px avatar (`UserCircle`), icon-only with `aria-label`. Not a business button.
...
Login and register are not operational pages. They do **not** use sidebar + top bar, and they are not a seventh chrome. Canonical: `frontend/src/components/layout/auth-card-layout.tsx` with `frontend/src/components/views/login.tsx` and `register.tsx`.
