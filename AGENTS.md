...
- **Gateway auth:** the OpenAI-compatible API enforces auth. Set `NEBULA_AUTH_TOKENS=dev-token:admin` (format `token:role`) and call with `Authorization: Bearer dev-token`, or set `NEBULA_AUTH_DISABLED=1` for dev.
