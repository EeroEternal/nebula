...
const messages: Record<Locale, Record<string, string>> = {
  zh: {
    ...
    'gateway.title': '网关观测',
    'gateway.subtitle': '查看网关流量、可靠性、延迟与防护指标',
    ...
    'cells.subtitle': '只读接入原生 Gateway / Ingress，不管理内部 worker',
    'cells.list': '已注册 Cell',
    'cells.loading': '加载中',
    ...
    'cells.registerHint': '将对 base_url 做 OpenAI /v1/models 探针。删除仅移除 Nebula 声明，不会停止外部服务。',
    ...
    'cells.dataSourceNote': '指标来源为 Cell Ingress，不写入 /stats/ 扩缩热路径',
    ...
    'governance.subtitle': '镜像兼容矩阵、硬件库存、模型 SLO、Benchmark、租户配额与成本',
    ...
    'governance.insufficient': '流量不足或缺少 scrape，不会假绿达标',
    'governance.benchmark': 'Benchmark 结果',
    'governance.benchmarkEmpty': '暂无 run；可用 scripts/benchmark/run_benchmark.py --ingest 写入',
    ...
    'governance.tenantsHint': 'token 绑定格式 NEBULA_AUTH_TOKENS=tok:role:tenant_id；开启 NEBULA_MULTI_TENANT=1 后 Gateway 执行租户准入',
    'governance.tenantsEmpty': '暂无租户；创建后写入 etcd /tenants/',
